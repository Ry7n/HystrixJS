...
function timeout(promise, timeMs) {
    ...
    return new _anyPromise2["default"](function (resolve, reject) {
        ...
        return doFinally(promise.then(resolve, reject), function () {
...
var Command = (function () {
    function Command(_ref) {
        ...
        var fallback = _ref$fallback === undefined ? function (err) {
    ...
    _createClass(Command, [{
        ...
        value: function execute() {
            ...
            return _anyPromise2["default"].resolve().then(function () {
                if (_this.requestVolumeRejectionThreshold != 0 && _this.metrics.getCurrentExecutionCount() >= _this.requestVolumeRejectionThreshold) {
                    return _this.handleFailure(new Error("CommandRejected"));
                }
                if (_this.circuitBreaker.allowRequest()) {
                    return _this.runCommand.apply(_this, _arguments);
                } else {
                    _this.metrics.markShortCircuited();
                    return _this.fallback(new Error("OpenCircuitError"));
                }
            });
        }
    }, {
        ...
        value: function runCommand() {
            ...
            promise = promise.then(function (res) {
                _this2.handleSuccess(start);
                return res;
            })["catch"](function (err) {
                return _this2.handleFailure(err);
            });

            return doFinally(promise, function () {
                return _this2.metrics.decrementExecutionCount();
            });
        }
    }, {
        key: "handleSuccess",
